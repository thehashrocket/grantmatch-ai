...
function computeStatus(deadline: Date | null, rawStatus?: string): { status: $Enums.GrantStatus; closedAt: Date | null } {
  const now = new Date();
  const normalizedStatus = rawStatus?.toLowerCase() ?? '';
  const isExplicitlyClosed = ['closed', 'inactive', 'expired'].some((term) => normalizedStatus.includes(term));
  const isClosedByDate = deadline !== null && deadline.getTime() < now.getTime();

  if (isExplicitlyClosed || isClosedByDate) {
    return { status: $Enums.GrantStatus.CLOSED, closedAt: deadline ?? now };
  }

  if (['active', 'open', 'posted', 'forecasted', 'upcoming'].some((term) => normalizedStatus.includes(term))) {
...
function buildSourceKeyFromFields(fields: Array<string | number | null | undefined>): string {
  const canonical = fields
    .map((field) => (field === null || field === undefined ? '' : String(field).trim().toLowerCase()))
    .join('|');
...
class GrantImportService {
  ...
  async upsertCaliforniaGrant(input: CaliforniaGrantInput): Promise<void> {
    ...
    if (deadlineDate) {
      ...
      const type = mapToEnumType(input.deadline, ['FIXED', 'ONGOING', 'ROLLING', 'TBD', 'CLOSED', 'UNKNOWN']);
    ...
    if (openDateDate) {
      ...
      const type = mapToEnumType(input.openDate, ['FIXED', 'ONGOING', 'ROLLING', 'TBD', 'CLOSED', 'UNKNOWN']);
    ...
    const portalIdNumber =
      typeof input.portalId === 'string' ? (Number.isNaN(parseInt(input.portalId, 10)) ? null : parseInt(input.portalId, 10)) : input.portalId;
    const sourceRecordId = input.sourceRecordId || (portalIdNumber !== null && portalIdNumber !== undefined ? String(portalIdNumber) : undefined);
    const sourceKey =
      input.sourceKey ||
      (!sourceRecordId
        ? buildSourceKeyFromFields([input.title, input.url, input.grantor, openDateDate?.toISOString(), deadlineDate?.toISOString()])
        : undefined);
    ...
    const grantData = {
      ...
      estimatedTotalFunding: BigInt(parseFundingAmount(input.estimatedTotalFunding)),
  ...
  async upsertFederalGrant(input: FederalGrantInput): Promise<void> {
    ...
    const sourceKey =
      ...
      (!input.sourceRecordId
        ? buildSourceKeyFromFields([input.title, input.url, input.grantor, openDate?.toISOString(), closeDate?.toISOString(), input.number])
        : undefined);
  ...
  private mapRowToRecord(headers: string[], row: string[]): CaliforniaCsvRow | null {
  ...
  private mapCaliforniaCsvRecord(record: CaliforniaCsvRow): CaliforniaGrantInput | null {
    ...
    const fundsDisbursement = combineValueWithNote(record.FundingMethod, record.FundingMethodNotes);
    const matchFunding = combineValueWithNote(record.MatchingFunds, record.MatchingFundsNotes);
    ...
    return {
      ...
      grantor: cleanValue(record.AgencyDept || record.FundingSource || record.AgencyURL) || 'Unknown',
      portalId,
      ...
      sourceRecordId: sourceRecordId || (portalId ? String(portalId) : undefined),
      sourceKey: cleanValue(record.sourceKey),
    };
  }

  private pickSourceRecordId(record: CaliforniaCsvRow): string | undefined {
    const preferredKeys = ['portal_id', 'portalId', 'opportunity_id', 'record_id', '_id', 'id', 'PortalID'];
