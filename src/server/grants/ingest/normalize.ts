...
export function computeSourceKey(fields: Array<string | number | undefined | null>): string {
...
export function computeStatus(deadline: Date | undefined, rawStatus?: string): { status: $Enums.GrantStatus; closedAt: Date | null } {
  ...
  const isClosedByText = ['closed', 'inactive', 'expired', 'ended'].some((marker) => norm.includes(marker));
  ...
  return norm ? { status: $Enums.GrantStatus.UNKNOWN, closedAt: null } : { status: $Enums.GrantStatus.OPEN, closedAt: null };
}
