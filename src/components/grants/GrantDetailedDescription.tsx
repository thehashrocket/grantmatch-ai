...
export default function GrantDetailedDescription({ description }: GrantDetailedDescriptionProps) {
                        ...
                        <CardContent className="space-y-4">
                            {projectSections.map((section, sectionIndex) => (
                                <div key={sectionIndex}>
                                    {Object.entries(section).map(([projectType, projectDescription], entryIndex) => (
    ...
    const renderObjectFormat = (jsonData: GrantJsonData) => {
        return (
            <div className="space-y-6">
                ...
                {jsonData.eligibleApplicants && jsonData.eligibleApplicants.length > 0 && (
                    <Card>
                        <CardHeader>
                            ...
                            <ul className="space-y-3">
                                {jsonData.eligibleApplicants.map((applicant, index) => (
                                    <li key={index} className="flex items-start">
                                        ...
                                        <span className="text-sm text-muted-foreground leading-relaxed">{applicant}</span>
                                    </li>
                                ))}
                            </ul>
                        </CardContent>
                        ...
                        <CardContent>
                            <ul className="space-y-3">
                                {jsonData.eligibleProjects.map((project, index) => (
                                    <li key={index} className="flex items-start">
                                        <span className="inline-block w-2 h-2 bg-blue-500 rounded-full mt-2 mr-3 flex-shrink-0" />
                                        <span className="text-sm text-muted-foreground leading-relaxed">{project}</span>
                                    </li>
                                ))}
                            </ul>
                        </CardContent>
                    ...
                    <Card>
                        ...
                        <CardContent className="space-y-4">
                            {jsonData.applicationInfo.applicationDeadline && (
                                <div>
                                    <h4 className="font-medium text-sm mb-1">Application Deadline</h4>
                                    <p className="text-sm text-muted-foreground">{jsonData.applicationInfo.applicationDeadline}</p>
                                </div>
                            )}
