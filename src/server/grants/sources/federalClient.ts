...
export async function postJsonWithRetry<T>(
	...
	return postWithRetry<T>(url, JSON.stringify(body), {
		...opts,
		headers: { 'Content-Type': 'application/json', ...(opts.headers ?? {}) },
	});
}

export async function postFormWithRetry<T>(
	url: string,
	formBody: string,
	opts: FetchOptions = {},
): Promise<T> {
	return postWithRetry<T>(url, formBody, {
		...opts,
		headers: {
...
export function parseFederalDateMMDDYYYY(
