...
export type GrantRecord = Prisma.GrantGetPayload<{ select: typeof grantSelect }>;

export interface GrantRepository {
  findWithFilters(filters: GrantSearchFilters): Promise<GrantRecord[]>;
  findWithFiltersPaginated(filters: GrantSearchFilters, page: number, pageSize: number): Promise<{ grants: GrantRecord[]; total: number }>;
...
export class PrismaGrantRepository implements GrantRepository {
  async findWithFilters(filters: GrantSearchFilters): Promise<GrantRecord[]> {
    ...
    const whereClause = Object.keys(whereConditions).length > 0 ? whereConditions : undefined;
  ...
  async findWithFiltersPaginated(filters: GrantSearchFilters, page: number, pageSize: number): Promise<{ grants: GrantRecord[]; total: number }> {
    const whereConditions = this.buildWhereConditions(filters);
    const whereClause = Object.keys(whereConditions).length > 0 ? whereConditions : undefined;
  ...
  private buildWhereConditions(filters: GrantSearchFilters): Prisma.GrantWhereInput {
    ...
    if (filters.minDeadline) {
      ...
      const deadlineFilter: Prisma.GrantWhereInput['deadline'] = { gte: minDeadline };
