...
export async function upsertGrantFromNormalized(
  ...
  const statusTransition = determineStatusTransition(existing.status as $Enums.GrantStatus, normalized.status);
  ...
  const changeType =
    ...
    (contentChanged ? 'UPDATED' : 'STATUS_CHANGED');
...
async function findExistingGrant(prisma: PrismaLike, normalized: NormalizedGrantInput): Promise<Grant | null> {
...
): { changed: boolean; type?: GrantChange['changeType']; nextStatus?: $Enums.GrantStatus; closedAt?: Date | null } {
  if (current === next) return { changed: false };

  if (current === $Enums.GrantStatus.OPEN && next === $Enums.GrantStatus.CLOSED) {
    return { changed: true, type: 'CLOSED', nextStatus: next, closedAt: new Date() };
  }

  if (current === $Enums.GrantStatus.CLOSED && next === $Enums.GrantStatus.OPEN) {
    return { changed: true, type: 'REOPENED', nextStatus: next, closedAt: null };
  }
