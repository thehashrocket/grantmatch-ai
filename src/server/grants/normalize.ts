...
const candidateLists = {
  sourceRecordId: ['opportunity_id', 'grant_id', 'portal_id', 'portalId', 'record_id', '_id', 'id'],
  ...
  description: ['details', 'description', 'full_description', 'grant_description'],
...
export function pickField(record: Record<string, unknown>, candidates: string[]): unknown {
...
function computeSourceKey(fields: Array<string | number | undefined | null>): string {
  const canonical = fields
    .map((field) => (field === undefined || field === null ? '' : String(field).trim().toLowerCase()))
    .join('|');
...
export function computeStatus(deadline: Date | undefined, rawStatus?: string): { status: $Enums.GrantStatus; closedAt: Date | null } {
  const now = new Date();
  const norm = rawStatus?.toLowerCase() ?? '';
  const isClosedByText = ['closed', 'inactive', 'expired'].some((marker) => norm.includes(marker));
  ...
  if (['open', 'posted', 'forecasted', 'upcoming', 'active'].some((marker) => norm.includes(marker))) {
...
export function normalizeCaliforniaRecord(record: Record<string, unknown>): NormalizeResult | null {
  const rawSourceRecordId = normalizeString(pickField(record, candidateLists.sourceRecordId));
  ...
  const stateAgency = normalizeString(pickField(record, candidateLists.grantor)) ?? undefined;
  const purpose = normalizeString(pickField(record, candidateLists.purpose));
  const description = normalizeString(pickField(record, candidateLists.description));
  const eligibleApplicants = normalizeList(pickField(record, candidateLists.eligibleApplicants));
  const eligibleGeographies = normalizeString(pickField(record, candidateLists.eligibleGeographies));
  ...
  const opportunityType = normalizeString(pickField(record, candidateLists.opportunityType));
  const currentAsOf = normalizeDate(pickField(record, candidateLists.currentAsOf));
