...
import { type GrantMatch, type PaginatedGrantMatches } from '@/lib/types/grant';
...
export interface GrantService {
  ...
  searchGrantsPaginated(filters: GrantSearchFilters, page: number, pageSize: number): Promise<PaginatedGrantMatches>;
...
export class GrantServiceImpl implements GrantService {
  ...
  async searchGrantsPaginated(filters: GrantSearchFilters, page: number, pageSize: number): Promise<PaginatedGrantMatches> {
    // Get paginated raw grants from repository
    const { grants, total } = await this.grantRepository.findWithFiltersPaginated(filters, page, pageSize);
