...
export async function fetchCkanSchema(resourceId: string): Promise<unknown> {
  ...
  if (!res.ok || !json.success) {
    ...
    throw new Error(`Failed to fetch CKAN schema: ${res.status} ${res.statusText} ${text ? `- ${text}` : ''}`);
  }
  return json.result ?? null;
}

export async function fetchCkanPage(resourceId: string, offset: number, limit = DEFAULT_PAGE_SIZE): Promise<{ records: Record<string, unknown>[]; total: number }> {
  ...
  if (!res.ok || !json.success) {
    throw new Error(`Failed to fetch CKAN records: ${res.status} ${res.statusText}`);
  }
