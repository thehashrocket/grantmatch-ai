import { Prisma, type Grant, $Enums } from '@/prisma/generated/client';
import { normalizeOppStatusToGrantStatus, parseFederalDateMMDDYYYY } from '@/server/grants/sources/federalClient';
...
function extractAttachments(raw: unknown): Omit<Prisma.GrantAttachmentCreateManyInput, 'grantId'>[] {
  ...
  for (const folder of raw as SynopsisAttachmentFolder[]) {
    const folderAttachments = (folder?.synopsisAttachments ?? []) as SynopsisAttachment[];
    ...
    for (const attachment of folderAttachments) {
      ...
      const sizeBytes = Number.isFinite(parsedSize) ? Math.trunc(parsedSize) : undefined;

      attachments.push({
        upstreamId: attachment?.id != null ? String(attachment.id) : undefined,
        fileName,
        mimeType: typeof attachment?.mimeType === 'string' ? attachment.mimeType : undefined,
        description: typeof attachment?.fileDescription === 'string' ? attachment.fileDescription : undefined,
        sizeBytes,
...
export function mapFederalApply07(details: FederalDetails, grant: Grant) {
  ...
  const fundingDescLinkUrl = synopsis.fundingDescLinkUrl ?? synopsis.synopsisURL ?? null;
  ...
  const { status, closedAt } = normalizeOppStatusToGrantStatus(synopsis.oppStatus, closeDate, now);
  ...
  const grantPatch: Partial<Prisma.GrantUncheckedUpdateInput> = {
    ...
    closedAt: status === $Enums.GrantStatus.CLOSED ? closedAt ?? closeDate ?? grant.closedAt ?? null : null,
    lastSeenAt: now,
  };
